...
for ((;;))
do
   test=`curl -XPUT -H "Content-Type:application/json" -d "@data.json" http://10.10.1.57:8765/api/admin/agent/heartbeat/agent_mac -s -w %{http_code}`
   echo "$(date +%Y%m%d-%H:%M:%S) ${test}" |tee -a result.log
   sleep 1
   endtime=$(date +%s)
   if [ $(expr $endtime - $starttime) -gt 310 ];then
     break
   fi
done
