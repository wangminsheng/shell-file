...
if [ "$?" = "0" ]; then
  ...
  # mv $CONFIRM_FOLDER/$STATION/$MBSN_P.start $CONFIRM_FOLDER/$STATION/$MBSN_P.fail
   send_log "CPU stress $testloop loop test Fail"
   lhand -z $mnt_monitor_path -l -s -L "Fail: $MBSN_P CPU stress test Fail"
   show_exit
fi
...
source ./ECC_check.sh
  ...
  lhand -z $mnt_monitor_path -l -s -L "$MBSN CPU stress test start"
CPU_stress_loop=4
...
while [ $testloop -lt $CPU_stress_loop ]
 do 
  ...
  lhand -z $mnt_monitor_path -l -s -L "$MBSN_P CPU stress test finished"
